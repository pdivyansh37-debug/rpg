  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      if (!isSupabaseConfigured) {
        ...
        setErrorMessage(
          'Supabase credentials not configured in .env.local yet. Running in offline/guest mode.'
        );
        setIsLoading(false);
        return;
      }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-cyan-400 bg-cyan-950/60 shadow-[0_0_15px_rgba(0,240,255,0.4)]">
            ...
            {errorMessage && (
              <div className="mb-4 rounded-2xl border border-rose-500/50 bg-rose-950/50 p-3 text-xs text-rose-300">
                {errorMessage}
              </div>
            )}
            ...
            {currentUser && !currentUser.isGuest ? (
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-emerald-500/40 bg-emerald-950/20 p-4 space-y-3">
                  ...
                  <div className="pt-2 border-t border-emerald-950 text-[11px] text-slate-400 flex items-center justify-between">
                    <span className="flex items-center gap-1">
                      <Cloud className="w-3 h-3 text-cyan-400" /> Auto-Saving Quests
                    </span>
                ...
                >
                  ...
                  <span>{isLoading ? 'Disconnecting...' : 'Disconnect Google Account'}</span>
                </button>
              </div>
            ) : (
              /* Guest Login View with Google Sign-In */
              <div className="space-y-4">
                <div className="rounded-2xl border border-indigo-950 bg-[#120d2c] p-4 text-xs space-y-2 text-slate-300">
                  <div className="flex items-center gap-2 font-bold text-cyan-300">
                    <Sparkles className="w-4 h-4 text-cyan-400" />
                    <span>Link Your Google Account</span>
                  </div>
                  <p className="leading-relaxed text-slate-400">
                    Sign in with Google to sync your character progression, stats, unlocked loot,
                    and habit streaks across all your devices in real-time.
                  </p>
                </div>
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="group relative w-full flex items-center justify-center gap-3 rounded-2xl bg-white px-5 py-3.5 text-sm font-bold text-slate-900 shadow-[0_0_20px_rgba(255,255,255,0.3)] transition-all hover:bg-slate-100 hover:shadow-[0_0_30px_rgba(255,255,255,0.5)] active:scale-[0.98] disabled:opacity-50"
                >
                ...
                <div className="pt-2 text-center text-[11px] text-slate-500">
                  <span>Playing as Guest Operator. Your data is stored locally.</span>
                </div>
              </div>
            )}
          </motion.div>
